...
export const BubblePlayer: FunctionComponent<BubblePlayerProps> = ({ onSwipeUp, currentTrack, togglePlayPause, isPlaying, onPlayNextClick, onPlayPrevClick, volume, setVolume, isSearchScope, liked, onLikeClick, onTrackClick }) => {
    ...
    return (
        <div ref={ref} class='flex flex-col-reverse sm:flex-row  sm:w-[840px] h-[11rem] sm:h-[90px] -0 p-4 gap-5 sm:gap sm:mx-auto sm:items-center justify-between rounded-[28px] sm:rounded-full backdrop-blur-[60px] bg-white bg-opacity-15 bubble-player'>
            ...
            <BubbleTrackInfo classes='sm:w-96' currentTrack={currentTrack} isSearchScope={isSearchScope} liked={liked} onLikeClick={onLikeClick} onTrackClick={onTrackClick} />
