...
export const TrackUploader: FunctionalComponent<TrackUploaderProps> = ({ uploadedTracksDetails, uploadFiles, inProcessTracks }) => {
    ...
    return (
        <div class='h-full flex flex-col'>
            <h2 className="flex text-white text-3xl font-bold">Uploader</h2>

            <div onDragLeave={() => dragAndDropMode.value = false} onDragOver={onFileDrag} onDrop={onFileDrop} class={`flex h-full ${uploadedTracksDetails.length > 0 ? '' : ''} flex-1 flex-col my-5 items-center justify-center w-full border-2 border-gray-300 border-dashed rounded-lg bg-transparent transition-all duration-200 ease-in`}>
            ...
            <div class={`${(uploadedTracksDetails.length > 0) ||  (inProcessTracks.length > 0) ? 'h-[510px]' : 'h-0'} transition-all duration-200 ease-in`}>
                <ScrollAndLoadList>
