const buildPalette = (colorsList) => {
  ...
  export const getKek = (file, setRgbs, setRgbs1) => {
    ...
    fileReader.onload = () => {
        image.onload = () => {
          ...
          const orderedByColor = orderByLuminance(quantColors).map(rgbToHex);
          const hslColors = convertRGBtoHSL(quantColors).map(hslToHex);

        //   setRgbs(quantColors.map(rgbToHex))
          setRgbs(orderedByColor)
          setRgbs1(hslColors)
